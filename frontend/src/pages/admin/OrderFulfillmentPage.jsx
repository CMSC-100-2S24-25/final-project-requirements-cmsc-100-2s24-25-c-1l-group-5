...
export default function OrderFulfillmentPage() {
  ...
  useEffect(() => {
    ...
    if (activeFilterTab === "0")
      ...
      filtered = filtered.filter((o) => o.status === 2);
  ...
  // Safe access for counts
  const safeData = Array.isArray(data) ? data : [];
  const pendingCount = safeData.filter((o) => o.status === 0).length;
  const completedCount = safeData.filter((o) => o.status === 1).length;
  const cancelledCount = safeData.filter((o) => o.status === 2).length;
          ...
          <Card className="flex flex-col w-full h-[70vh]">
            ...
            <CardFooter className="flex items-center justify-between mt-auto">
              <div className="text-sm text-muted-foreground">
                ...
                of {safeFilteredData.length} orders
              </div>
